...
in
  with lib; let
      ...
      foldl (
        defs: module: let
          findDefs = x:
            ...
            then findDefs x.content
            else if x ? contents
            then lib.foldl (defs: x: defs ++ findDefs x) [] x.contents
            else [];
    ...
    typeCheckedInputDefs = let
      wrongTypeDefs = filter (def: builtins.typeOf def.value != "set") inputDefs;
    in
      if wrongTypeDefs == []
      then inputDefs
      else throw "A definition for option `inputs' is not of type `attribute set of raw value'. Definition values:${options.showDefs wrongTypeDefs}";
  ...
  in
    foldlAttrs (
      ...
      in
        if areDefsEqual
        then inputs
        else throw "The input `${inputName}' has conflicting definition values:${options.showDefs defs}"
    )
    uncheckedInputs
    uncheckedInputs
